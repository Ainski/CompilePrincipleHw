...
\hline
程浩然 & 2351579 & 一遍式架构合并、目标代码生成器、9.3 元组元素访问、测试与AI辅助章节 \\
\hline
黄艺鑫 & 2253797 & 规则 7.x 表达式化、IR 嫁接、x86 翻译表设计、报告主体撰写 \\
\hline
韦畅 & 2352628 & 实验报告撰写、规则对照与落实清单、测试用例设计、文档与参考资料整理 \\
\hline
\end{tabular}
\end{table}
...
\placeholder{系统总体架构图：自上而下展示完整数据流——类Rust源代码 \(\rightarrow\) Flex 词法分析（TokenStream）\(\rightarrow\) 递归下降语法分析（AST）\(\rightarrow\) SemanticAnalyzer 单次遍历（语义检查 + 四元式 IR 同步生成）\(\rightarrow\) CodeGenerator 翻译 \(\rightarrow\) x86-64 AT\&T 汇编(.s) \(\rightarrow\) as 汇编 + ld 链接 \(\rightarrow\) 可执行程序。左侧标注 Pipeline 协调层，右侧标注各阶段输出文件（lexer-output / parser-output / ir-output / asm-output）。}{图2-1 系统总体架构图}
...
\placeholder{函数调用栈帧示意图：展示调用者与被调用者的栈帧布局——调用者 push 参数到寄存器、call 指令压入返回地址；被调用者 push \%rbp、建立帧、局部变量在 -8(\%rbp)/-16(\%rbp)... 处、形参从 \%rdi 等寄存器读入栈槽；返回时 \%rax 持返回值，mov \%rbp,\%rsp + pop \%rbp + ret 恢复。}{图5-1 函数调用栈帧布局}
...
\placeholder{运行截图：终端中 fibonacci 程序从源码生成汇编、as 汇编、ld 链接、运行 ./fib 后 echo \$? 返回 55 的完整命令序列与输出。}{图7-1 目标代码端到端运行验证截图}

\placeholder{目标代码截图：CodeGenerator 生成的 fibonacci.s 文件内容（AT\&T 语法），展示函数 prologue、参数寄存器传递、while 循环的 LABEL/JZ/JMP 翻译、RETURN 与函数 epilogue。}{图7-2 生成的 x86-64 汇编代码截图}
...
\begin{lstlisting}[caption={fibonacci(10) 端到端验证}]
$ ./parser --input testfiles/asm_fib.rs \
    --asm-output fib.s --entry fibonacci --entry-arg 10 -q
...
全部语法语义规则（0.1\(\sim\)9.3）均落实并通过测试，包括本阶段新增的规则 7.0\(\sim\)7.4（函数表达式块、选择/循环表达式）与 9.3（元组元素访问）。优秀评定所需 ⭐ 数远超 7 颗门槛。
...
本课程设计过程中，团队使用了 AI 辅助工具（Claude）参与多个环节。本章如实说明 AI 的使用情况、团队对 AI 生成代码的审查与修复过程，以及由此获得的启示。

\subsection{AI 使用环节}

AI 主要参与以下环节（团队始终负责方案决策、代码审查与正确性验证）：

\begin{enumerate}
    \item \textbf{架构设计探讨}：在 M1 架构合并阶段，AI 提供了"嫁接合并 vs 三钩子 driver"的方案对比分析，团队经评估选定嫁接合并方案。
    \item \textbf{代码生成}：AI 协助生成 \texttt{CodeGenerator}（目标代码翻译器）、\texttt{visitIfExpr}/\texttt{visitLoopExpr}（表达式 visitor）等模块的初版代码。
    \item \textbf{调试辅助}：在 IR 对比验证、临时变量编号偏移等问题上，AI 协助定位根因。
    \item \textbf{重构执行}：AI 执行了一边嫁接、9.3 补齐等机械性重构。
\end{enumerate}

\subsection{审查与修复案例}

\subsubsection{案例一：临时变量编号偏移}

AI 生成的嫁接代码在赋值左值处理时，\texttt{visitDerefExpr} 仍调用 \texttt{newTemp()} 分配编号（虽因 \texttt{emit\_enabled} 关闭而不输出指令），导致后续 \texttt{ARRAY\_LIT} 的结果临时变量编号比 IRGenerator 多 1，IR golden 不一致。

\textbf{团队审查发现}后，修复方案为：\texttt{newTemp()} 在 \texttt{emit\_enabled} 为 false 时直接返回空字符串、不分配编号。修复后 IR 与 golden 逐行一致。该案例说明：AI 生成的代码可能在"不可见"的副作用（如编号分配）上偏离预期，必须通过差分测试（IR 对照）捕获。

\subsubsection{案例二：块表达式末尾的隐式 return 误传播}

AI 实现规则 7.2（函数体末尾表达式作隐式 return）时，\texttt{block\_tail\_as\_return} 标志在进入函数体后未对嵌套块复位，导致 \texttt{if} 表达式的 then/else 块末尾表达式也被误判为函数 return，生成了多余的 \texttt{RETURN} 指令。

\textbf{团队审查发现}后，引入局部变量 \texttt{amFuncBody}：\texttt{visitBlock} 进入时记录当前标志值并立即复位为 false，确保只有函数体直接块的末尾表达式触发隐式 return，嵌套块不受影响。修复后 7.x 测试通过且既有行为零回归。

\subsubsection{案例三：复合类型整体赋值缺陷}

补齐 9.3 元组元素访问时，运行测试 \texttt{tuple\_test(5)} 返回错误结果。\textbf{团队定位}发现：\texttt{t = (1,2,3)} 的整体赋值经临时变量 + 单字节 \texttt{ASSIGN} 只 copy 了首元素，导致 \texttt{t.1}、\texttt{t.2} 读到垃圾值。这是既有的潜在缺陷（数组也有），因前期未运行复合类型程序而未暴露。

修复方案为引入 \texttt{array\_lit\_target}：当左值是数组/元组变量、右值是数组/元组字面量时，让 \texttt{ARRAY\_LIT} 直接以目标变量名为 result，跳过临时变量与 \texttt{ASSIGN}，所有元素正确就位。该修复同时改善了数组的整体赋值。

\subsection{启示与心得}

\begin{enumerate}
    \item \textbf{AI 生成代码必须经过差分验证}：嫁接合并的正确性完全依赖 IR golden 逐行对比，仅靠"编译通过 + 部分测试"无法发现编号偏移、多余指令等细微缺陷。
    \item \textbf{AI 擅长机械重构，不擅长边界语义}：一边嫁接的代码搬运高效准确，但"赋值左值不求值"、"函数体 vs 嵌套块"等语义边界需人工把关。
    \item \textbf{测试驱动开发的价值}：\texttt{asm\_tests.sh} 的运行验证在 9.3 补齐时直接暴露了潜伏的复合赋值缺陷，证明"可运行验证"是不可替代的正确性保障。
    \item \textbf{架构清晰是 AI 协作的基础}：本项目清晰的 AST 契约与模块边界，使 AI 能在明确约束下生成可审查的代码。
\end{enumerate}
...
通过本课程设计，团队深入理解了编译器从源代码到目标代码的完整构造过程，掌握了一遍编译的工程实现、四元式中间代码设计、x86-64 汇编生成与调用约定、以及 AI 辅助编程的审查方法论。
