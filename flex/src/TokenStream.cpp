...
TokenStream::TokenStream() {
    // Optional: open output file for debugging
    // output.open("tokenstream_debug.txt");
    ofs.open(lexeroutput_file, ios::out);
    if (!ofs.is_open()) {
        cerr << "Failed to open output file: " << lexeroutput_file << "\n";
        exit(1);
    }
}
...
bool TokenStream::addToken(const Token& t) {
    cout<<t.type<<"\t"<<t.category<<"\t"<<t.value<<"\n";
    ofs <<t.type<<"\t"<<t.category<<"\t"<<t.value<<"\n";
    toks.push_back(t);
    return true;
}
