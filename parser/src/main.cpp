...
public:
    ...
    void PrintToScreem(int depth = 0) const {
        string indent(depth * 2, ' ');
        if (isLeaf) {
            cout << indent << label << "\n";
        } else {
            cout << indent << "<" << label << ">\n";
            for (auto& c : children) c->PrintToScreem(depth + 1);
        }
    }
    ...
    void PrintToFile(ofstream& ofs, int depth = 0) const {
        ...
        if (isLeaf) {
            ...
            ofs << indent << "<" << label << ">\n";
            for (auto& c : children) c->PrintToFile(ofs, depth + 1);
        }
    }
};
...
int main(int argc, char* argv[]) {
    ...
    app.add_option("--parser-output",parser_output, "Output file for parser results") ->required();
    ...
    string input = "";
    app.add_option("--input", input, "Input file path (Original Rust File)")->required();
