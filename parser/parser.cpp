...
public:
    ...
    // Consume and return; throw if category doesn't match.
    ...
    bool check(const string& cat) const { return peek().category == cat; }

    // True if the token at offset has the given type (not category)
    ...
    // Consume if category matches; return true if consumed.
